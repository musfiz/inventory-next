...
import type { PurchaseBySupplierReport, PurchaseBySupplierRow } from '@/types/report.types';
...
export default function PurchaseBySupplierPage() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await reportService.purchaseBySupplier(params);
      setData(res as unknown as PurchaseBySupplierReport);
    } catch (e: any) {
  ...
  const cards: SummaryCard[] = data ? [
    { label: 'Total Spend', value: formatCurrency(data.summary.total_spend), color: 'red' },
    { label: 'Top Supplier', value: data.summary.top_supplier, color: 'green' },
  ] : [];

  const columns: ReportColumn[] = [
    { key: 'supplier_name', header: 'Supplier' },
    { key: 'po_count', header: 'PO Count', format: 'number', align: 'right' },
    { key: 'total_items', header: 'Items', format: 'number', align: 'right' },
    ...
    { key: 'total_due', header: 'Total Due', format: 'currency', align: 'right' },
  ];

  return (
    <ReportLayout title="Purchase by Supplier" icon={Building2}
      filters={
      ...
      actions={<ReportExportBar
        onExportPDF={() => { if (reportRef.current) exportToPDF(reportRef.current, 'purchase-by-supplier'); }}
        onExportExcel={() => { if (data) exportColumnsToExcel(data.data, columns.map(c => ({ key: c.key, label: c.header })), 'purchase-by-supplier', 'Purchase by Supplier'); }}
        onExportCSV={() => { if (data) exportColumnsToCSV(data.data, columns.map(c => ({ key: c.key, label: c.header })), 'purchase-by-supplier'); }}
        onPrint={() => printReport('report-print', 'Purchase by Supplier Report')}
        disabled={!data} />}
