...
export default function CouponsPage() {
  ...
  const columns: ColumnDef<Coupon>[] = [
    ...
    { id: 'actions', header: 'Actions', meta: { width: '10%' }, cell: ({ row }) => <div className="flex gap-1"><button className="p-1 text-blue-600 hover:text-blue-900 dark:text-blue-400 rounded cursor-pointer" title="Edit" onClick={()=>handleEdit(row.original)}><Edit className="w-3.5 h-3.5"/></button><button className="p-1 text-red-600 hover:text-red-900 dark:text-red-400 rounded cursor-pointer" title="Delete" onClick={()=>handleDelete(row.original)}><Trash2 className="w-3.5 h-3.5"/></button></div> },
  ];
